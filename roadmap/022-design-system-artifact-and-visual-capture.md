...
3. **Project-dependent implementation.** The capture mechanism will differ by stack (React Storybook, Playwright visual regression, test-render-helpers, etc.), but the contract — give me a screenshot of this component in this state — must be universal.
...
- The artifact must include: active preset, resolved token values, component patterns in use, and a pointer to the full guide for deep reference.
...
- The blueprint must specify the interface contract (what to call, what it returns) but leave framework choice to the project.
- Captured screenshots must land in a predictable, documented path (this work unit defines its own convention; there is no separate evidence-policy unit).
...
- The artifact format must be plain markdown or a structured comment block that Claude Code can parse without custom tooling.
