---
title: "Run a Clean Full Implement Review Fix Demo on Roadmap Todo"
status: ready
...
# Run a Clean Full Implement Review Fix Demo on Roadmap Todo

## Intent

Produce one clean, contiguous PR timeline in this repository that exercises the full GitHub Actions workflow loop:

- implement opens a fresh PR from a roadmap work unit
- implementation includes bot-generated screenshot evidence from the rendered app
...
- Update `pilots/roadmap-todo/` only for the implementation and fix stages.
...
- Include screenshot-backed evidence in the resulting PR metadata using workflow-owned bot comments or PR body content.
- Prefer the PR timeline to show both the initial screenshot evidence and any refreshed screenshot evidence added after the fix.
...
- If the fix materially changes the UI, refresh the screenshot evidence on the same PR.
...
- [ ] The implementation PR includes bot-generated screenshot evidence that makes the visual change understandable without local checkout.
...
- [ ] The PR timeline shows refreshed screenshot evidence after the fix if the UI changed.
