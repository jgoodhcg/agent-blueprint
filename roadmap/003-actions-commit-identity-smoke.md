...
- The change should state that autonomous GitHub Actions implementation commits use `Agent Blueprint Implementer <agent-blueprint-implementer@users.noreply.github.com>` as the git author/committer while model attribution remains in `Co-authored-by` and `AI-*` trailers.
...
- [ ] The resulting commit is authored and committed by `Agent Blueprint Implementer <agent-blueprint-implementer@users.noreply.github.com>`.
